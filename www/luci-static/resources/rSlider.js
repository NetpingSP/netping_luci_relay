(function () {
	...
	var RS = function (conf) {
		...
		this.cls = {
			...
			selected: 	'rs-selected',
			selected_from_start: 	'rs-selected-from-start',
			selected_to_end: 	'rs-selected-to-end',
			pointer: 	'rs-pointer',
			scale: 		'rs-scale',
			noscale:	'rs-noscale',
			tip: 		'rs-tooltip',
			result_message: 'rs-result-message'
		};
	...
	RS.prototype.createSlider = function () {
		...
		this.selected_from_start = createElement('div', this.cls.selected_from_start);
		this.selected_to_end = createElement('div', this.cls.selected_to_end);
		this.pointerL = createElement('div', this.cls.pointer, ['dir', 'left']);
		this.scale = createElement('div', this.cls.scale);
		this.result_message = createElement('div', this.cls.result_message);
		...
		this.slider.appendChild(this.selected_from_start);
		this.slider.appendChild(this.selected_to_end);
		this.slider.appendChild(this.scale);
		this.slider.appendChild(this.pointerL);
		this.input.parentNode.appendChild(this.result_message);
	...
	RS.prototype.createScale = function (resize) {
		...
		for (var i = 0, iLen = this.conf.values.length; i < iLen; i++) {
			...
			if (!this.conf.labels) {
				...
				if (i/4 == Math.round(this.conf.values[i]) && (Math.round(this.conf.values[i])) % 4 == 0) {
					ins.innerHTML = this.formatTime(this.conf.values[i]);
				}
			}
	...
	RS.prototype.formatTime = function(hours) {
			var minutes = "000000" + (hours - Math.floor(hours)) * 60
		return Math.floor(hours) + ":" + minutes.substr(minutes.length-2);
	}

	RS.prototype.showResultMessage = function() {
		this.result_message.innerHTML = "Запрещено отправлять уведомления с " + this.formatTime(this.conf.values[this.values.start]) + "  до " + this.formatTime(this.conf.values[this.values.end]) + "!"
	}
	...
	RS.prototype.setValues = function (start, end) {
		...
		if (this.conf.range) {
			if (this.conf.tooltip) {
				this.tipL.innerHTML = this.formatTime(this.conf.values[this.values.start]);
				this.tipR.innerHTML = this.formatTime(this.conf.values[this.values.end]);
			}
		...
		if (this.values.start > this.conf.values.length - 1) this.values.start = this.conf.values.length - 1;
		...
		if(this.values.start <= this.values.end) {
			...
			this.selected_to_end.style.width = (this.conf.values.length - 1 - this.values.start) * this.step + 'px';
